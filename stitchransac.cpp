...
main (int argc, char **argv)
{
		...
		pcl::PointCloud<pcl::PointXYZ>::Ptr alignedxyz(new pcl::PointCloud<pcl::PointXYZ>);
		copyPointCloud(*object_aligned, *alignedxyz);
		*scenexyz += *alignedxyz;
		pcl::visualization::CloudViewer viewer("Test");
		viewer.showCloud(scenexyz);
		while(!viewer.wasStopped()){}
		PointCloud<PointXYZ>::Ptr object_fullxyz (new PointCloud<PointXYZ>);
		PointCloud<PointXYZ>::Ptr scene_fullxyz (new PointCloud<PointXYZ>);
		...
		*scene_orig += *object_orig;
		...
		io::savePCDFileBinary("merged_align_normal.pcd", *scene_orig);
